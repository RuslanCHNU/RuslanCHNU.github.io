...
export default function RateHistogram({ data = [], binCount = 12, height = 200, unit = "грн/день", desc = "Історична швидкість: середня сума (грн), що збиралась щоденно у кампаній." }) {
  const bins = useMemo(() => makeBins(data, binCount), [data, binCount]);

  if (!data || data.length === 0)
    return <Typography>Немає даних</Typography>;
  ...
  return (
    <ResponsiveContainer width="100%" height={400}>
      <BarChart data={bins}>
        ...
        <YAxis />
        <Tooltip
          content={<ChartTooltip unit={unit} desc={desc} />}
        />
        <Bar dataKey="count" />
      </BarChart>
    </ResponsiveContainer>
